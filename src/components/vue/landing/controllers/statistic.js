...
export class Stat {
	constructor(ticker, key) {
		...
		this.key = key;
		this.setHistoryPath();
		this.history = reactive(history[ticker][key]);
	...
	setHistoryPath() {
		if (!history[this.ticker]) Vue.set(history, this.ticker, reactive({}));
		if (!history[this.ticker][this.key]) Vue.set(history[this.ticker], this.key, reactive([]));
	}
}
