	...
	<div class="tx-display">
		<span class="tx-tags">
			<span v-if="source.h" :class="$root.darkMode ? 'is-black' : 'is-info'" class="tag">
				<img
					:src="config.baseUrl + 'static/img/singles/house_logos/' + source.h.toLowerCase() + '.png?v=' + $root.appVersion"
				/>
			</span>
		</span>
		...
		<span class="tx-tags right">
			...
			<span v-if="source.e && source.e.op_return && (source.chain != 'DASH' || source.char != 'dash')" :class="$root.darkMode ? 'is-black' : 'is-info'" class="tag"
				>OP_R</span
			>
...
export default {
	...
	mounted() {
		console.log("Source data:", this.source);
	},
};
</script>